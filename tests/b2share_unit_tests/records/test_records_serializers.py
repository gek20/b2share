...
def make_v1_record(test_records_data):
    ...
    record['creators'] = [
        {'creator_name': 'Glados, R.'}, 
        {'creator_name': 'Cube, Companion', 'family_name': 'Cube', 'given_name': 'Companion'}
    ]
    return pid, record
...
def test_records_serializers_eudatcore(app, test_records_data):
    with app.app_context():
        ...
        # ]
        assert [t.text for t in xml.xpath('//publishers')[0].xpath('//publisher')] == [record['publisher']]
